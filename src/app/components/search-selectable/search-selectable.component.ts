...
export class SearchSelectableComponent implements ControlValueAccessor, OnInit {
  ...
  async pesquisarItens(event: any) {
    ...
    if (this.base.isNullOrWhiteSpace(data)) {
      return;
    }

    try {
      if (!this.isMultiple) {
        if (this.base.isNullOrWhiteSpace(data.ngModel[this.itemValueField])) {
          return;
        }
      }
    } catch (error) {
    ...
    if (!this.base.isNullOrWhiteSpace(data.ngModel) && data.ngModel !== {}) {
  ...
  keyDown(event: any) {
    if (!this.base.isNullOrWhiteSpace(this.value) && event.keyCode === 13) {
      return;
    }
