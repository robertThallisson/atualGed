...
export class RadioMbiComponent implements ControlValueAccessor, OnInit {
  ...
  onClick(index: number) {
    if (this.multiplo) {
      this.value[index][this.fieldValue] = isNullOrWhiteSpace(this.value[index][this.fieldValue]) ? true : !this.value[index][this.fieldValue];
    } else {
  ...
  getNome(item: any) {
    return this.base.getValueField(item, this.fieldName);
  }
